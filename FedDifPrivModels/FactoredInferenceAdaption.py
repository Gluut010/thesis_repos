...
class FactoredInference:
    ...
    def interior_gradient(self, measurements, total, lipschitz = None,c = 1,sigma = 1,callback=None, iM = 100, dConvergence = 10**-1):
        ...
        c0 = c
        l = sigma/L
        dLossOld, _ = self._marginal_loss(x) 
        for k in range(1, self.iters+1):
            ...
            #check for convergence every iM iters
            if (k % iM) == 0:
                dLossNew, _ = self._marginal_loss(x)
                if dLossOld - dLossNew < dConvergence:
                    if self.log:
